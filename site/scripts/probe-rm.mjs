...
const server = createServer(async (req, res) => {
  try {
    ...
    if (p === '/40-YEARS' || p.startsWith('/40-YEARS/')) p = p.slice('/40-YEARS'.length) || '/';
...
const rm = await p1.evaluate(() => ({
  theatreOn: document.documentElement.classList.contains('theatre-on'),
...
const nojs = await p2.evaluate(() => ({
  theatreHidden: getComputedStyle(document.getElementById('bg-theatre')).display,
  preloaderHidden: document.getElementById('preloader')?.hidden,
  ledger: document.querySelector('[data-count]')?.textContent?.trim(),
}));
